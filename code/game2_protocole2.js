...
function updateDisplay(){
   switch(level.resolutionType){
      case 'next_pressed':
         ...
         else if(lvlCount<begin_exp || lvlCount>end_exp || lvlCount%2!=0){
...
audioPlayer.onplay = function(){
   ...
   if(level.resolutionType != 'next_pressed' && ecoID==1 || level.resolutionType === 'seqs_solved_with_timer') {
      revealElement(oButton);
      revealElement(xButton);
      revealElement(skipButton);
      revealElement(seqDisplayer[0]);
...
function resetLevel(){
   switch(lvlCount){
      ...
      case 7 :
         ...
         pageText.innerHTML += "<p id='formNasa'> Cliquez sur <a id='nasaButton' href='NASA.html' target='_blank' >ce lien</a> pour accéder au formulaire. </p>"
      break;
      case 8:
      ...
      case 9 :
         ...
         pageTitle.textContent = "Niveau 0 - reproduction de séquences.";
      ...
      default:
         if(lvlCount >= begin_exp && lvlCount <= end_exp){
            if(lvlCount % 2 != 0){
               pageTitle.textContent = "Niveau "+((lvlCount-7)/2-1)+"/9";
               ...
               pageTitle.textContent = "Bilan Niveau "+((lvlCount-8)/2-1);
   ...
   if(level.resolutionType == 'sound_played' || level.resolutionType == 'seqs_solved_with_timer') {
      let data = new URLSearchParams();
...
document.onkeydown = function(e) {
   var key = e.key;
   if(lvlCount==5 || lvlCount >= begin_exp && lvlCount <= end_exp && lvlCount % 2 != 0){
